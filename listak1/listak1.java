...
class Listak1
{
    ...
    {
        int count = words.size();
        List<String> frontX = new ArrayList<String>();
        List<String> lastX = new ArrayList<String>();
        for(int i = 0; i < count; i++){
            String actual = words.get(i);
            if(actual.charAt(0) == 'x'){
                frontX.add(words.get(i));
            }
            else{
                lastX.add(words.get(i));
            }
        }
        Collections.sort(frontX);
        Collections.sort(lastX);
        List<String> sortedList = new ArrayList<String>();
        for(int i = 0; i < frontX.size(); i++){
            sortedList.add(frontX.get(i));
        }
        for(int i = 0; i< lastX.size(); i++){
            sortedList.add(lastX.get(i));
        }
        return sortedList;
    }
